...
//                // TODO: REMOVE
                ...
                // Insert ScheduleEventDb (FK - activity_id)
                ScheduleEventDb scheduleEvent = new ScheduleEventDb(savedActivity.getId(),
                        Calendar.getInstance(), Calendar.getInstance(), weekdaysPicker.getSelectedDays());
                db.scheduleEventDao().insert(scheduleEvent);

                // Retrieve ScheduleEventDb
                ScheduleEventDb savedScheduleEvent = db.scheduleEventDao().findScheduleEventById(scheduleEvent.getId());

                // Insert RuleDb (FK - activity_id, setting_id)
                RuleDb rule = new RuleDb(savedActivity.getId(), SettingType.BLUETOOTH, 0);
                db.ruleDao().insert(rule);

                // Retrieve RuleDb
                RuleDb savedRule = db.ruleDao().findRuleById(rule.getId());
                System.out.println();
            }
        });
